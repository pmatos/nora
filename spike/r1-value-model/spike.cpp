...
enum EK {
  LIT, CLOSLIT, VAR, IFE,
  ADD, SUB, BOXV, UNBOXV, CONSV, CARV, EQ0,
  RECUR, CALLV,
};
...
static nr_value interp_loop(int64_t N) {
  Expr n{VAR}; n.var = 0;
  Expr sum{VAR}; sum.var = 1;
  Expr zero{EQ0}; zero.a = &n;

  Expr one{LIT}; one.lit = nr_fixnum(1);
  Expr nm1{SUB}; nm1.a = &n; nm1.b = &one;

  Expr boxn{BOXV}; boxn.a = &n;         // (box n)  -> garbage
  Expr ubox{UNBOXV}; ubox.a = &boxn;    // (unbox (box n)) = n
  Expr consss{CONSV}; consss.a = &sum; consss.b = &sum; // (cons sum sum) -> garbage
  Expr carp{CARV}; carp.a = &consss;    // (car (cons sum sum)) = sum
  Expr sump{ADD}; sump.a = &ubox; sump.b = &carp;       // n + sum

  Expr recur{RECUR}; recur.args = {&nm1, &sump};
  Expr body{IFE}; body.a = &zero; body.b = &sum; body.c = &recur;
...
// ---------------------------------------------------------------------------
static nr_value add_captured(nr_value self, int64_t argc, const nr_value *argv) {
  assert(argc == 1);
...
int main(int argc, char **argv) {
  ...
  printf("nr_value = %zu bytes;  ObjHeader = %zu;  NrPair = %zu;  NrBox = %zu;  "
         "NrClosure = %zu\n\n",
  ...
  printf("[%.4s] interp(%lld) = ", nrt_eq(ri, rc) ? "ok" : "FAIL", (long long)N);
  nrt_write(ri);
  printf(" ; compiled = ");
  nrt_write(rc);
  printf(" ; expected N(N+1)/2 = %lld\n", (long long)expected);
  if (!nrt_eq(ri, rc) || nr_fixnum_val(ri) != expected) fails++;
  ...
  Expr cl{CLOSLIT}; cl.lit = clos;
  Expr a5{LIT}; a5.lit = five;
  Expr call{CALLV}; call.a = &cl; call.args = {&a5};
  nr_value r_interp = evalNT(&call, {});
  printf("[%.4s] closure (capture 10) applied to 5: interp=%lld compiled=%lld\n",
  ...
  if (!nrt_eq(r_interp, r_compiled) || nr_fixnum_val(r_interp) != 15) fails++;
  ...
  printf("[%.4s] GC collected: churned %.2f GiB into a %.1f MiB live heap "
         ...
         (double)heap_after / (1024 * 1024), (double)churn / (double)heap_after);
  if (!collected) fails++;
  (void)total_before;
  (void)heap_before;
